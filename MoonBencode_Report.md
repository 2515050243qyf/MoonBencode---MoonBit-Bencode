...
- **自动化测试覆盖**: 编写了边界条件测试与集成测试，覆盖整数（包含负数）、字节串、列表、字典、嵌套复合结构、规范校验、JSON、路径查询、info_hash、种子文件布局、磁力链接、策略校验、安全限制和真实场景样例。当前本地报告为 42 个测试全部通过，生产源码 5,696 行、测试源码 691 行；Linux/macOS/Windows CI 和 native 后端流程已写入 README 与工作流。
